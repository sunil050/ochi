...
function Navbar() {
  ...
  const handleScroll = () => {
    if (window.scrollY > lastScrollY) {
      // Scroll down
      setIsVisible(false);
      setIsScrollingUp(false); // Remove blur when scrolling down
    } else {
      // Scroll up
      setIsVisible(true);
      setIsScrollingUp(true); // Apply blur effect when scrolling up
    }
  ...
  return (
    <div
      className={`fixed z-20 w-full px-20 py-8 font-["Neue Montreal"] flex justify-between items-center transition-transform duration-500 ${
        isVisible ? "translate-y-0" : "-translate-y-full"
      } ${
        isScrollingUp ? "backdrop-blur-md bg-opacity-30" : ""
      }`} // Added blur effect
      style={{ transition: "backdrop-filter 0.5s ease-in-out" }}
    >
      <div className="logo">
      <svg
          width="72"
        ...
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              ...
              href={`#${item.replace(/\s+/g, "").toLowerCase()}`} // Adjusted for anchor linking
            >
              {item}
            </a>
          )
